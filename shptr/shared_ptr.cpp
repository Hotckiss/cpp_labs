...
shared_ptr::shared_ptr(const shared_ptr& other) : storage_(other.storage_) { storage_->incr(); }
...
bool shared_ptr::isNull() const { return !storage_->getObject(); }
void shared_ptr::reset(Matrix* obj) {
    if (ptr() == obj) return;
    shared_ptr tmp(obj);
    std::swap(storage_, tmp.storage_);
}
Matrix* shared_ptr::operator->() const { return this->ptr(); }
Matrix& shared_ptr::operator*() const { return *this->ptr(); }
